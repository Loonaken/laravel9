              ...
              <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form method="POST" action="{{route('owner.images.update' , ['image' => $image->id])}}">
                    ...
                    <div class="my-4">
                      ...
                      <div class="p-2 mb-8 w-1/3 md:w-1/4">
                        <div class="relative">
                        <x-thumbnail :filename="$image->filename" type="products" />
                        </div>
                      </div>
                      <div class="flex p-2 justify-around w-full">
                        <button type="button" onclick="location.href='{{route('owner.images.index')}}'" class="flex mx-auto text-black bg-gray-300 border-0 py-2 px-8 focus:outline-none hover:bg-gray-200 rounded text-lg">戻る</button>
                ...
                <form id="delete_{{$image->id}}" method="post" action="{{route('owner.images.destroy', ['image'=>$image->id])}}">
                  ...
                  <a href='#' data-id="{{$image->id}}" onclick="deletePost(this)" type="submit" class=" text-white bg-red-500 border-0 py-2 px-2 focus:outline-none hover:bg-red-600 rounded text-base">削除する</a>
                </div>
              </form>
              </div>
          </div>
